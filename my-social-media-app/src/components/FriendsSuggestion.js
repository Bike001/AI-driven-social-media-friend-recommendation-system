      ...
      {suggestions.length > 0 ? (
        suggestions.map((suggestion, index) => (
          <div key={index} className="suggestion">
            {suggestion.username} {/* Adjust if the key is different */}
            {/* Add more user info and an 'add friend' button if needed */}
            <button onClick={() => sendFriendRequest(suggestion.username)}>+</button>
          </div>
        ))
      ) : (
        <p>No friends suggestions available.</p> // Shows a message if no suggestions
      )}
    </div>
  );
}
